...
Essa faixa, é um intervalo de valor definido pela discretização seção \ref{cap:refTeor:sec:discret}, onde o intervalo escolhido, seria a faixa que representa os valores que se repetem com a maior frequência no atributo. A exemplo disso, tem-se um vetor de elementos já discretizados, ${\vec{a}_i=\{1,1,1,2,2,2,2,3,3\}}$, onde ${(\vec{a})}$ representa todos os elementos do  ${(a_i)}$ onde $i\leqslant m$. Neste vetor o valor que mais se repete é o número 2, então, a \textbf{faixa 2}  é a escolhida junto com o atributo ${(a_i)}$ para compor o rótulo.
