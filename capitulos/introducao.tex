...
A formação do problema desta pesquisa nasce a partir do trabalho realizado por \citeonline{LOPES2014}, que se dedicou a estudar a possibilidade de realização de rotulação automática de grupos utilizando para isso dois algoritmos: i) Um para realizar a formação de grupos através de algoritmo não supervisionado (K-means); e ii) utiliza o algoritmo supervisionado (Redes Neurais Artificiais - RNA) para fazer a rotulação de grupos. Assim, partindo do estudo já realizado, este trabalho se dedica a realizar rotulação de grupos de dados a partir de outros algoritmos supervisionados não testados e fazer um comparativo entre eles.
