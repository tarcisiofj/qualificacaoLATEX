...
As bases de dados escolhidas para este trabalho foram: Seeds, Iris, Glass, Wine Quality. Essas bases são bastantes utilizadas em vários trabalhos\footnote{https://archive.ics.uci.edu/ml/datasets/glass+identification, https://archive.ics.uci.edu/ml/datasets/iris, https://archive.ics.uci.edu/ml/datasets/wine+quality, https://archive.ics.uci.edu/ml/datasets/seeds}  possibilitando análises de seus comportamentos através de seus resultados, visto que, todas as bases são classificadas, servindo de referência por critérios comparativos para os resultados desta pesquisa, uma vez que esta pesquisa trabalhará com os \textit{clusters} já formados e não na criação desses \textit{clusters}. Outra motivo desta seleção, é mitigar o máximo a utilização das técnicas de \textit{Feature Engineering} \cite{Casari2018}, que são técnicas de manipulação de dados empregue na base para aplicação do algoritmo de aprendizado de máquina utilizada na fase de preparação dos dados, que é um fase inicial na execução de aprendizado de máquina. 

As tabelas \ref{tab:rot:seeds:nb} a \ref{tab:rot:glass:cart}, apresentadas neste capítulo, representam os resultados dos algoritmos executados a cada base de dados, e a disposição detas tabelas seguem o seguinte formato: a primeira coluna (\textbf{Cluster}) é um número que representa cada cluster de forma sequencial, a segunda coluna (\textbf{Rótulos}) representado pelo par, \textbf{Atributo} e \textbf{Faixa}, o quais definem o rótulo do cluster respectivo. A coluna \textbf{Relevância} tem a importância de informar o quanto, em porcentagem, aquele atributo é relacionado aos demais, na definição do rótulo daquele cluster. A quinta coluna, \textbf{Fora da Faixa}, exibe o erro em quantidade de elementos que não estão naquela faixa definida naquele rótulo, e por último a \textbf{Acurácia do Cluster}, o qual exibe, em porcentagem, o valor de acertos de elementos que são representados pelo rótulo. 
