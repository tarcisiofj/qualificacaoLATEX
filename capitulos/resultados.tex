...
\begin{table}[!h]
\centering
\caption{Matriz de Atributos Importantes do algoritmo CART}
...
Através da tabela \ref{tab:rot:iris:nb}  são exibidos os resultados da rotulação de dados, e um fato a se considerar nesse resultado, são as colunas de \textbf{Relevância} com valor de 100\% (cem por cento) e \textbf{Fora da Faixa} com 0 (zero) erros no cluster 1, implicando em uma acurácia de 100\% (cem por cento). O cluster 2 e cluster 3 obtiveram rótulos distintos, cada um com grau de relevância acima de 80\% em  cada grupo.
